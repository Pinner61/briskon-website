...
// Updated Auction interface with null support for time fields
interface Auction {
  id: string; // Required
  productname?: string;
  title?: string;
  ...
  startprice?: number;
  scheduledstart?: string | null; // Allow null for time
  ...
  useEffect(() => {
    const fetchAuctionDetails = async () => {
      try {
        ...
        const updatedAuction: Auction = { ...json.data, id: auctionId, participants }; // Ensure id is set
  ...
  const getMinimumBid = () => {
    ...
    if (auction?.bidcount && auction?.bidcount > 0 && auction?.currentbid) {
      if (auction.bidincrementtype === "percentage" && auction.percent) {
        ...
        minimumBid = (auction.currentbid ?? 0) + (auction.minimumincrement ?? 0);
      }
    }
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                    ...
                    <span>
                      ...
                      {auction.bidincrementtype === "percentage" && auction.percent && auction.currentbid
                        ? ((auction.currentbid ?? 0) * (auction.percent / 100)).toLocaleString()
                        : (auction.minimumincrement ?? 0).toLocaleString()}
                      {" ("}
